...
checkpoint = "my_new_zhen_bleurt_checkpoint/export/bleurt_best/1622036938"
...
# 获取参考答案
refs = []
with open("./data68/references/CE/1.CE ref.txt", encoding="utf8") as f:
    refs.append(f.readlines())
with open("./data68/references/CE/2.CE ref.txt", encoding="utf8") as f:
    refs.append(f.readlines())
with open("./data68/references/CE/3.CE ref.txt", encoding="utf8") as f:
    refs.append(f.readlines())
p_refs = []
n=0
for ref in refs:
...
# 测试文件夹    
file_path = "./data68/C-to-E-Sentence/"

# 遍历文件夹，获取文件名，存放在列表files中
files = []
for _,_,k in os.walk(file_path):
    files=k
...
# 保存结果
import json
with open("68_ce_bleurt_3ref_score.json", "w", encoding="utf8") as f:
